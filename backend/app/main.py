...
def ensure_apple_compatible(path: Path, job: Job) -> Path:
    ...
    command = [imageio_ffmpeg.get_ffmpeg_exe(), "-y", "-hide_banner", "-loglevel", "error", "-filter_threads", "1", "-i", str(path), "-map", "0:v:0", "-map", "0:a:0?", "-map_metadata", "0"]
    command += ["-c:v", "copy"] if video_ok else [
